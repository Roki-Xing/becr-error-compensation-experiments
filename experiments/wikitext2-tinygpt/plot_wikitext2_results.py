...
def main() -> None:
    ...
    runs = sorted(runs, key=lambda r: order.index(r["mode"]) if r["mode"] in order else 999)
    ...
    ax_tloss, ax_vloss, ax_gn = axs[0, 0], axs[0, 1], axs[0, 2]
    ax_scale, ax_e, ax_ppl = axs[1, 0], axs[1, 1], axs[1, 2]

    # Plot only seed=0 curves for readability.
    for mode in order:
        run0 = next((r for r in runs if r["mode"] == mode and int(r.get("seed", 0)) == 0), None)
        if run0 is None:
            continue
        tr = run0["traces"]
        x = _arr(tr, "step")

        ax_tloss.plot(x, _arr(tr, "train_loss"), linewidth=1.6, color=color.get(mode), label=label.get(mode, mode))

        vloss = _arr(tr, "val_loss")
        if np.isfinite(vloss).any():
            ax_vloss.plot(x, vloss, linewidth=1.6, color=color.get(mode))

        gn = _arr(tr, "grad_norm")
        if np.isfinite(gn).any():
            ax_gn.plot(x, gn, linewidth=1.4, color=color.get(mode))

        phi_p95 = _arr(tr, "phi_raw_p95")
        s_p95 = _arr(tr, "s_p95")
        if np.isfinite(phi_p95).any():
            ax_scale.plot(x, phi_p95, linewidth=1.2, color=color.get(mode), linestyle="--")
        if np.isfinite(s_p95).any():
            ax_scale.plot(x, s_p95, linewidth=1.5, color=color.get(mode))

        en = _arr(tr, "residual_norm")
        if np.isfinite(en).any():
            ax_e.plot(x, en, linewidth=1.4, color=color.get(mode))

    ax_tloss.set_title("(a) Train loss (seed=0)")
    ax_tloss.set_xlabel("step")
    ax_tloss.set_ylabel("CE loss")
    ax_tloss.grid(True, alpha=0.3)
    ax_tloss.legend(loc="best", fontsize=9)

    ax_vloss.set_title("(b) Valid loss (seed=0)")
    ax_vloss.set_xlabel("step")
    ax_vloss.set_ylabel("CE loss")
    ax_vloss.grid(True, alpha=0.3)

    ax_gn.set_title("(c) ||grad|| (logged, diagnostic)")
    ...
    # (f) Final valid perplexity aggregated over seeds.
    by_mode: dict[str, list[float]] = {}
    for r in runs:
        by_mode.setdefault(r["mode"], []).append(float(r.get("final_valid_ppl", float("nan"))))
    modes = [m for m in order if m in by_mode]
    means = [float(np.nanmean(by_mode[m])) for m in modes]
    stds = [float(np.nanstd(by_mode[m], ddof=1)) if len(by_mode[m]) > 1 else 0.0 for m in modes]
    xs = np.arange(len(modes))
    ax_ppl.bar(xs, means, yerr=stds, capsize=4, color=[color.get(m, "#999999") for m in modes], alpha=0.9)
    ax_ppl.set_xticks(xs, [label.get(m, m) for m in modes], rotation=25, ha="right")
    ax_ppl.set_title("(f) Final valid perplexity (mean±std over seeds)")
    ax_ppl.grid(True, axis="y", alpha=0.3)

    fig.suptitle("WikiText-2 (raw) TinyGPT (mechanism diagnostic)")
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(FIG_DIR / "wikitext2_tinygpt_coordproj__MAIN.png", dpi=240)
    fig.savefig(FIG_DIR / "wikitext2_tinygpt_coordproj__MAIN.pdf")
    plt.close(fig)

    print("Wrote:", str(FIG_DIR / "wikitext2_tinygpt_coordproj__MAIN.png"))

    # Per-layer heatmaps (seed=0, final trace).
    ...
    fig2, axs2 = plt.subplots(1, 3, figsize=(15.8, 8.0))
    ...
    fig2.suptitle("WikiText-2 per-layer mechanism diagnostics (weights only)")
    fig2.tight_layout(rect=[0, 0, 1, 0.95])
    fig2.savefig(FIG_DIR / "wikitext2_tinygpt_coordproj__PER_LAYER.png", dpi=240)
    fig2.savefig(FIG_DIR / "wikitext2_tinygpt_coordproj__PER_LAYER.pdf")
    plt.close(fig2)

    print("Wrote:", str(FIG_DIR / "wikitext2_tinygpt_coordproj__PER_LAYER.png"))
